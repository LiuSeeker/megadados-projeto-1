...
def acha_post_info_por_id(conn, id_post):
    with conn.cursor() as cursor:
        cursor.execute("SELECT id_usuario, titulo, texto, url_imagem FROM post WHERE id_post=%s", (id_post))
        res = cursor.fetchone()
        if res:
            return res[0]
        else:
...
def lista_post_id_por_palavra(conn, palavra):
    with conn.cursor() as cursor:
        cursor.execute("SELECT id_post FROM post WHERE texto LIKE %{$%s}", (palavra))
        res = cursor.fetchall()
        if res:
            return tuple(x[0] for x in res)
        else:
...
def update_post_titulo(conn, id_post, titulo):
    with conn.cursor as cursor:
        try:
            cursor.execute("UPDATE post SET titulo=%s where post=%s", (titulo, id_post))
        except pymysql.err.IntegrityError as e:
...
def update_post_texto(conn, id_post, texto):
    with conn.cursor as cursor:
        try:
            cursor.execute("UPDATE post SET texto=%s where post=%s", (texto, id_post))
        except pymysql.err.IntegrityError as e:
            raise ValueError(f'Erro ao dar update em post')
...
def update_post_url_imagem(conn, id_post, url_imagem):
    with conn.cursor as cursor:
        try:
            cursor.execute("UPDATE post SET url_imagem=%s where post=%s", (url_imagem, id_post))
        except pymysql.err.IntegrityError as e:
            raise ValueError(f'Erro ao dar update em post')
...
def update_post_ativo(conn, id_post, ativo):
    with conn.cursor as cursor:
        try:
            cursor.execute("UPDATE post SET ativo=%s where post=%s", (ativo, id_post))
        except pymysql.err.IntegrityError as e:
            raise ValueError(f'Erro ao dar update em post')
